	...
	, m_drag_enable(false)
{
	...
	//m_p_self = &m_m[GetCharInfo()->Name];

	m_p_client      = new Client(0x7F000001, 44444);
	m_p_spell_queue = new SpellQueue();
	m_p_chase       = new Chase();

	m_p_clients  = &(m_p_client->m_clients);
...
void MQ2Stats::CommandChase(PCHAR p_line)
{
	...
	if(args.size() > 0)
	{
		...
		{
			WriteChatf("Invalid chase argument: %s", p_line);
		}
	}
}
...
void MQ2Stats::CommandMissing()
{
	...
	for(auto it = m_p_clients->begin(); it != m_p_clients->end(); it++)
	{
...
void MQ2Stats::SetClientIndex(PCHAR index)
{
	...
	auto it = m_p_clients->find(index);

	if(it != m_p_clients->end())
	{
		m_p_data_index = &it->second;
	}
	else
	{
		m_p_data_index = nullptr;
