...
SpellData* SpellQueue::Front()
{
	return m_queued.size() ? const_cast<SpellData*>(&(*m_queued.begin())) : nullptr;
}
...
void SpellQueue::Push(SpellType& type, PSPAWNINFO p_target)
{
	...
	if(nullptr != p_spell && nullptr != p_target)
	{
...
void SpellQueue::Remove(SpellType& type, PSPAWNINFO p_target)
{
	SpellData data;
	data.type = type;
	data.p_target = p_target;

	auto it = m_queued.find(data);
...
void SpellQueue::AddSpell(PSPELL p_spell)
{
	...
	case SpellCategory::Heals:
	{
		...
		{
			PSPELL& p_curr = m_spells[SpellType::Resurrect];
	...
	case SpellCategory::HPBuffs:
	{
		...
		case SpellSubCategory::Shielding: // Shielding / Skin
		{
			break;
		}
...
SpellType SpellQueue::ToType(std::string& type_name)
{
...
bool SpellQueue::IsQueued(SpellType& type, PSPAWNINFO p_target)
{
	...
	return m_queued.find(data) != m_queued.end();
}
...
SpellQueue::SpellFailure SpellQueue::IsCastable(PSPELL p_spell, PSPAWNINFO p_caster, PSPAWNINFO p_target)
{
	...
	if(p_caster->ManaCurrent < static_cast<int32_t>(p_spell->ManaCost))
	{
