...
bool SpellCompare(const SpellData& left, const SpellData& right)
{
	// Elements of same type and target cannot be inserted
	if(left.type == right.type)
	{
		return left.p_target < right.p_target;
	}
...
private:
	...
	std::set<SpellData, decltype(&SpellCompare)> m_queued;
